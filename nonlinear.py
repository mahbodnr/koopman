...
import primal
from kooplearn._src.operator_regression.primal import evaluate_eigenfunction, estimator_eig
...
def cov(X: torch.Tensor, Y: Optional[torch.Tensor] = None) -> torch.Tensor:
    """
    Compute covariance matrix between X and Y (or X and X if Y is None).
    
    Args:
        X: Input tensor of shape (n_samples, n_features) or (n_features,)
        Y: Optional second input tensor of shape (n_samples, m_features) or (m_features,)
        
    Returns:
        Covariance matrix of shape (n_features, m_features) or (n_features, n_features) if Y is None
    """
    # Ensure X is at least 2D
    if X.dim() == 1:
        X = X.unsqueeze(0)
    if X.dim() > 2:
        raise ValueError(f"Input tensor has more than 2 dimensions ({X.dim()}).")
    ...
    if Y is None:
        ...
        if Y.dim() == 1:
            Y = Y.unsqueeze(0)
        if Y.dim() > 2:
            raise ValueError(f"Input tensor has more than 2 dimensions ({Y.dim()}).")
            
        if X.size(0) != Y.size(0):
            raise ValueError(
                f"Shape mismatch: the covariance between two tensors can be computed only if they have the same initial dimension. Got {X.size(0)} and {Y.size(0)}."
            )
...
class Nonlinear(BaseModel):
    ...
    def eig(
        ...
        if hasattr(self, "_eig_cache"):
            ...
            w, vl, vr = estimator_eig(self.U.numpy(), self.cov_XY.numpy())
        ...
        if eval_left_on is None and eval_right_on is None:
            ...
            return (
                w,
                evaluate_eigenfunction(phi_Xin_l, vl),
                evaluate_eigenfunction(phi_Xin_r, vr),
            )
